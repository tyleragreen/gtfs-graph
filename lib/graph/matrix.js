...
class Matrix {
  ...
  setRow(origin, distancesToOrigin) {
    distancesToOrigin.forEach((distance, destination) => {
      this.set(origin, destination, distance);
    });
  }
  
  makeCopy() {
    const newMatrix = [];
    this.matrix.forEach(function(rowOfNodes) {
      newMatrix.push(rowOfNodes.slice());
    });
    return newMatrix;
  }
}
