...
var findCriticalEdges = function(graph, traverser) {
  const generations = 10;
  const mutationRate = 40;
  const populationSize = 10;
  const edgesPerSolution = 4;
  ...
  function calculateFitness(graph, solution) {
    const theoreticalGraph = graph.createNewGraphWithEdges(solution);
    const closeness = closenessCentrality(theoreticalGraph);
    
    return utils.mean(closeness);
  }
  ...
  const population = new Population({
    ...
    calculateFitness: calculateFitness.bind(null, graph),
  ...
  console.log('BEST AROUND', population.solutions[0]);
};
