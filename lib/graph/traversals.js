...
var transitRank = function(graph, traverser) {
  const initialRank = 0.001;
  let iterations = 1;
  let ranks;
  ...
  function updateRank(graph, originIndex) {
    ...
    for (let destIndex = 0; destIndex < graph.length(); destIndex++) {
      if (destIndex !== originIndex) {
        inNodeSummation += (ranks[destIndex] / graph.getShortestPath(originIndex, destIndex));
      }
    }
    
    console.log('summation', inNodeSummation);
    return inNodeSummation;
  }
...
var mergeTransferNodes = function(graph, socket) {
  ...
  function mergeEdges(edgeA, edgeB) {
    if (edgeA === 0 && edgeB === 0) {
      ...
      let weightA = edgeA.weight || 0;
      let weightB = edgeB.weight || 0;
