...
class Stop {
  ...
  mergeWith(stop) {
    ...
    const nameIsUnique = this.name.split('/').filter(name => name.trim().indexOf(stop.name.trim()) !== -1).length === 0;
    logger.info('name is unique?', nameIsUnique);
    logger.info('existing name', this.name);
    logger.info('non unique names',this.name.split('/').filter(name => name.trim().indexOf(stop.name.trim()) !== -1));
    logger.info('other',this.name.split('/').filter(name => name.trim().indexOf(stop.name.trim()) === -1));
    if (nameIsUnique) {
