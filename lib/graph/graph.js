...
class TransitGraph {
  ...
  constructor(edgeList, numNodes, stops) {
    ...
    this.calculateRandomWalks(200, PATH_DISTANCE);
  ...
  transitionProbability(j, origin, distance) {
    ...
    const numerator = randomWalks.filter(walk => walk[distance] === j);
  ...
  calculateRandomWalks(count, distance) {
    ...
    for (let node = 0; node < this.length(); node++) {
      ...
      const randomWalks = [];
      while (iteration--) {
        const path = this.randomWalk(node, distance);
  ...
  // A self-avoiding random walk from a given origin
  randomWalk(origin, distance) {
    if (distance <= 0) { throw new Error('Distance of a random walk must be positive'); }
    const path = [ origin ];
    ...
    while (distance--) {
      ...
      // Return the path early if our walk has reached a dead end
      if (unvisitedNodes.length === 0) { return path; }
      
      let nextNode = unvisitedNodes[Math.floor(Math.random() * unvisitedNodes.length)];
      path.push(nextNode);
