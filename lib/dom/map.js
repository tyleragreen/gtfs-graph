...
export default React.createClass({
  ...
  addRankLayers: function(stopGeoJson) {
    const thatMap = this.state.map;
    const ranks = stopGeoJson.features.map(stop => stop.properties.rank);
    const min = Math.min(...ranks);
    const max = Math.max(...ranks);
    const range = max - min;
    const firstQuarter = 0.25 * range + min;
    const secondQuarter = 0.5 * range + min;
    const thirdQuarter = 0.75 * range + min;
    
    const layers = [
      [min, 'rgba(0,255,0,0.5)', 50],
      [firstQuarter, 'rgba(0,204,0,0.6)', 60],
      [secondQuarter, 'rgba(0,153,0,0.7)', 70],
      [thirdQuarter, 'rgba(0,102,0,0.8)', 80]
    ];
