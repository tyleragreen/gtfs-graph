...
var edgesQuery = 'SELECT DISTINCT st1.stop_id AS origin, st2.stop_id AS destination, \'subway\' AS type \
                   FROM stop_times st1 \
                   JOIN stop_times st2 \
                    ON st1.trip_id=st2.trip_id \
                   WHERE st2.stop_sequence=(st1.stop_sequence+1) \
                  UNION \
                   SELECT from_stop_id AS origin, to_stop_id AS destination, \'transfer\' AS type \
                   FROM transfers \
...
var getStopPairs = function(client, stops, callback) {
  ...
  query.on('row', function(row) {
    if (row.type == "subway") {
      let new_row = {
        type: 'subway',
        edge: [stop_ids.indexOf(row.origin.substring(0, row.origin.length-1)), stop_ids.indexOf(row.destination.substring(0, row.destination.length-1))]
      };
      edgeList.push(new_row);
    } else if (row.type == "transfer") {
      let new_row = {
        type: 'transfer',
        edge: [stop_ids.indexOf(row.origin), stop_ids.indexOf(row.destination)]
      };
      edgeList.push(new_row);
    } else {
