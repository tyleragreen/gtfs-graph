...
var pageRank = function(graph, socket) {
  const mergedGraph = mergeTransferNodes(graph, socket);
...
var mergeTransferNodes = function(graph, socket) {
  ...
  function merge(loIndex, hiIndex) {
    let newStopInfo = {
      'stop_name': newStops[loIndex].stop_name + ' / ' + newStops[hiIndex].stop_name,
      'stop_lat': (newStops[loIndex].stop_lat + newStops[hiIndex].stop_lat) / 2,
      'stop_lon': (newStops[loIndex].stop_lon + newStops[hiIndex].stop_lon) / 2
    };
