...
var dfs = function(graph, node, traverser) {
  ...
  if (traverser) { traverser.summary('Stations Visited: ' + traverser.visitedNodes.length); }
};
...
var dijkstra = function(graph, start, end, traverser) {
  ...
  if (traverser) {
    ...
    let hours = Math.floor(S[end] / 3600);
    let minutes = Math.floor((S[end] % 3600) / 60);
    if (hours === 1) {
      traverser.summary('Duration: ' + hours + ' hour, ' + minutes + ' minutes');
    } else if (hours > 1) {
      traverser.summary('Duration: ' + hours + ' hours, ' + minutes + ' minutes');
    } else {
      traverser.summary('Duration: ' + minutes + ' minutes');
    }
  }
};
