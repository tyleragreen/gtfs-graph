...
class Population {
  ...
  sort() {
    ...
    const fitnessSnapshot = this.solutions.map(solution => that.calculateFitness(solution));
    
    this.solutions.sort(function(a,b) {
      const indexA = that.solutions.indexOf(a);
      const indexB = that.solutions.indexOf(b);
