...
class SystemManager {
  ...
  analyzeGraphs() {
    ...
    this.systems.forEach(function(system,index) {
      ...
      graph.ranks = {};
      graph.ranks[MODES.accessibility] = traversals.outwardAccessibility(graph);
      graph.ranks[MODES.pageRank] = traversals.pageRank(graph);
      graph.ranks[MODES.katz] = traversals.katzCentrality(graph);
      graph.ranks[MODES.closeness] = traversals.closenessCentrality(graph);
