...
exports.listen = function(SystemManager) {
  ...
  io.on('connection', function (socket) {
    ...
    socket.on(socketMsg.startPR, function(system) {
      ...
      //}
      if (typeof graph.mergedGraph === "undefined") {
        traversals.closenessCentrality(graph, graphTraverser);
      } else {
        traversals.closenessCentrality(graph.mergedGraph, graphTraverser);
      }
    });
    socket.on(socketMsg.getMode, function(system, mode) {
      graphTraverser.reset();
      logger.info(socketMsg.getMode);
      ...
      if (typeof graph.mergedGraph === "undefined") {
        ...
        graphToUse = graph.mergedGraph;
      }
      
      if (mode == 'PageRank') {
        traversals.pageRank(graphToUse, graphTraverser);
      } else if (mode == 'Closeness') {
        traversals.closenessCentrality(graphToUse, graphTraverser);
      } else if (mode == 'Katz') {
        traversals.katzCentrality(graphToUse, graphTraverser);
      } else if (mode == 'Accessibility') {
        traversals.outwardAccessibility(graphToUse, graphTraverser);
      } else {
