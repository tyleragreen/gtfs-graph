...
exports.listen = function(SystemManager) {
  ...
  router.get('/api/v0/graph/:system', function(req, res) {
    ...
    if (SystemManager.systemExists(systemId)) {
      ...
      // FIXME I NEED A MODE ENUM
      if (typeof mode !== "undefined" && !(mode !== "closeness" || mode !== "accessibility" || mode !== "pagerank" || mode !== "katz")) {
        res.send('Bad mode: '+mode);
        return;
      }
      ...
      if (typeof filter === "undefined") {
        ...
        res.send('Bad filter: ' + filter);
      }
    } else {
      res.send('Bad system: '+systemId);
    }
  });
  ...
  io.on('connection', function (socket) {
    ...
    socket.on(socketMsg.requestStops, function(system) {
      socket.emit(socketMsg.sendStops, SystemManager.getGraph(system, 'primary').stops);
    });
    
    socket.on(socketMsg.requestMergedStops, function(system) {
      socket.emit(socketMsg.sendMergedStops, SystemManager.getGraph(system, 'merged').stops);
    });
    
    socket.on(socketMsg.requestEdges, function(system) {
      socket.emit(socketMsg.sendEdges, SystemManager.getGraph(system, 'primary').getGeoJsonEdges());
    });
    
    socket.on(socketMsg.requestMergedEdges, function(system) {
      socket.emit(socketMsg.sendMergedEdges, SystemManager.getGraph(system, 'merged').getGeoJsonEdges());
    });
    ...
    socket.on(socketMsg.startDfs, function(system, stopId) {
      ...
      let graph = SystemManager.getGraph(system, 'primary');
    ...
    socket.on(socketMsg.getMode, function(system, mode) {
      ...
      const graph = SystemManager.getGraph(system, 'merged');
