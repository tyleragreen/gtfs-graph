...
exports.listen = function(SystemManager) {
  ...
  //=================================================
  // Graph API Endpoints
  router.get('/api/v1/graph/:system', function(req, res) {
    const systemId = req.params.system;
    
    if (SystemManager.systemExists(systemId)) {
      const type = req.query.type || 'primary';
      const mode = req.query.mode;
      res.send(SystemManager.getGraph(systemId, type).edgesAsGeoJsonFeatures());
    } else {
  ...
  io.on('connection', function (socket) {
    ...
    socket.on(socketMsg.requestEdges, function(system) {
      socket.emit(socketMsg.sendEdges, SystemManager.getGraph(system, 'primary').edgesAsGeoJsonFeatures());
    });
    
    socket.on(socketMsg.requestMergedEdges, function(system) {
      socket.emit(socketMsg.sendMergedEdges, SystemManager.getGraph(system, 'merged').edgesAsGeoJsonFeatures());
    });
