...
var Stop = require('../graph/stop');
var Route = require('../graph/route');
var traversals = require('../graph/traversals');
...
var getStopPairs = function(client, stops, callback) {
  ...
  query.on('row', function(row) {
    ...
    if (originId !== -1 && destId !== -1) {
      ...
      let existingEdges = edgeList.filter(function(edge) {
        return edge.origin === originId && edge.destination === destId;
      });
      ...
      if (existingEdges.length === 1) {
        if (row.duration < existingEdges[0].duration && row.duration !== 0) {
          existingEdges[0].duration = row.duration;
        }
      ...
      // If this procedure works correctly, we should never allow duplicate
      // (origin,destination) pairs into the graph
      } else {
        throw new Error('Duplicate edge detected during graph creation!');
      }
    }
  });
  query.on('end', function() {
  ...
  pg.connect(system.connectionString, function(err, client, done) {
    ...
    async.waterfall([
      ...
      function(err, stops, edgeList, numNodes) {
        ...
        if (mergedGraph) { mergedGraph.calculatePathLengths(); }
